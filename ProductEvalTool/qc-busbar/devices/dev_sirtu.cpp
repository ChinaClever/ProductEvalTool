...
bool Dev_SiRtu::readPduData()
{
    ...
    if(ret && mItem->addr == mRtuPkt->addr) { //回收地址和发送地址同
        ...
        box->zeroLineCur.ivalue = mRtuPkt->zeroLineCur.ivalue;
        box->zeroLineCur.imin = mRtuPkt->zeroLineCur.imin;
        box->zeroLineCur.imax = mRtuPkt->zeroLineCur.imax;
        box->zeroLineCur.ialarm = mRtuPkt->zeroLineCur.ialarm;

        box->volUnbalance = mRtuPkt->volUnbalance;
        box->curUnbalance = mRtuPkt->curUnbalance;

        box->totalPow.ivalue = mRtuPkt->totalPow.ivalue;
        box->totalPow.imin = mRtuPkt->totalPow.imin;
        box->totalPow.imax = mRtuPkt->totalPow.imax;
